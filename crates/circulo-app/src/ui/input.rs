//! shadcn/Base UI–style single-line text field (`Input`).
...
use gpui::{
    ...
    Focusable, FontWeight, GlobalElementId, InspectorElementId, IntoElement, KeyBinding, LayoutId,
    MouseButton, MouseDownEvent, MouseMoveEvent, MouseUpEvent, PaintQuad, Pixels, Point,
    SharedString, ShapedLine, Style, TextRun, UTF16Selection, Window,
};

use crate::theme::{
    ACCENT, BORDER, INPUT_BG, INPUT_BORDER_FOCUS, INPUT_HEIGHT_PX, TEXT, TEXT_MUTED,
};

actions!(
    text_input,
    [Backspace, Delete, Enter, Paste]
);
...
/// Field label (`FieldLabel` in shadcn).
pub fn field_label(label: impl Into<String>) -> impl IntoElement {
    div()
        .text_sm()
...
impl Element for TextInputElement {
    ...
    fn prepaint(
        ...
        let (display_text, color) = if input.content.is_empty() {
            (input.placeholder.clone(), TEXT_MUTED)
        } else {
            (input.content.clone(), TEXT)
        };
        let run = TextRun {
