...
- `POST /session` body `{}` → 200 `Session` (`id` matches `ses_*`). Defaults
  (agent `build`, model from user config) come from the server; we do not send
  model/agent.
- `POST /session/{sessionID}/prompt_async` body
  `{"parts": [{"type": "text", "text": "..."}]}` → **204 No Content**.
...
3. Zero or more assistant **steps**. Each step is its own assistant message
   ...
   - `part.type == "reasoning"` parts (Circulo does not model these; skip)
...
| everything else (`session.updated`, `session.diff`, `step-*`, `reasoning`, `server.*`, plugin/lsp/mcp noise, unknown future types) | skipped, never fatal |
...
- The session title is auto-updated by the server during the turn
  (`session.updated` carries a generated `title`). Out of scope here; noted for
  the open auto-title product decision (PRD §12).
