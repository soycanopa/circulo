 ...
 *
 * The renderer accesses these via `window.circulo.*`.
 * All methods return Promises (backed by `ipcRenderer.invoke`).
 */
contextBridge.exposeInMainWorld("circulo", {
	...
	cli: {
		/** Checks whether the `circulo` CLI command is installed. */
		isInstalled: () => ipcRenderer.invoke("cli:is-installed"),
		/** Installs the `circulo` CLI command (symlinks to /usr/local/bin). */
		install: () => ipcRenderer.invoke("cli:install"),
		/** Uninstalls the `circulo` CLI command. */
	...
	skills: {
		...
		install: (params: { ownerRepo: string; skillName?: string; target?: string }) =>
