 ...
 * from here instead of `circulo-server.ts` directly.
 *
 * In Electron mode, calls go through IPC to the main process.
 * In browser mode, calls go through HTTP to the Circulo server.
 */
 ...
 * The `circulo` object is exposed via `contextBridge.exposeInMainWorld`.
 */
export const isElectron = typeof window !== "undefined" && "circulo" in window
...
export async function fetchOpenCodeUrl(): Promise<{ url: string }> {
	...
	try {
		if (isElectron) {
			const info = await window.circulo.ensureOpenCode()
			log.info("OpenCode server URL resolved", { url: info.url })
			return { url: info.url }
		}
		const { fetchOpenCodeUrl: httpFetch } = await import("./circulo-server")
...
export async function resolveAuthHeader(
	...
	if (server.type === "remote" || server.type === "ssh") {
		...
		const password = await window.circulo.credential.get(server.id)
...
export async function fetchModelState(): Promise<ModelState> {
	if (isElectron) {
		return window.circulo.getModelState()
	}
	const { fetchModelState: httpFetch } = await import("./circulo-server")
...
export async function updateModelRecent(model: {
	...
	if (isElectron) {
		return window.circulo.updateModelRecent(model)
	}
	const { updateModelRecent: httpUpdate } = await import("./circulo-server")
 ...
 * In browser, pings the Circulo HTTP server.
 */
export async function checkBackendHealth(): Promise<boolean> {
	if (isElectron) {
		return true
	}
	const { checkServerHealth } = await import("./circulo-server")
...
export async function pickDirectory(): Promise<string | null> {
	if (isElectron) {
		return window.circulo.pickDirectory()
	}
...
export async function fetchGitBranches(directory: string): Promise<GitBranchInfo> {
	if (isElectron) {
		return window.circulo.git.listBranches(directory)
	}
...
export async function fetchGitStatus(directory: string): Promise<GitStatusInfo> {
	if (isElectron) {
		return window.circulo.git.getStatus(directory)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function gitCheckout(directory: string, branch: string): Promise<GitCheckoutResult> {
	if (isElectron) {
		return window.circulo.git.checkout(directory, branch)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function gitStashAndCheckout(
	...
	if (isElectron) {
		return window.circulo.git.stashAndCheckout(directory, branch)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function gitStashPop(directory: string): Promise<GitStashResult> {
	if (isElectron) {
		return window.circulo.git.stashPop(directory)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function getGitRoot(directory: string): Promise<string | null> {
	if (isElectron) {
		return window.circulo.git.getRoot(directory)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function fetchDiffStat(directory: string): Promise<GitDiffStat> {
	if (isElectron) {
		return window.circulo.git.diffStat(directory)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function gitCommitAll(directory: string, message: string): Promise<GitCommitResult> {
	if (isElectron) {
		return window.circulo.git.commitAll(directory, message)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function gitPush(directory: string, remote?: string): Promise<GitPushResult> {
	if (isElectron) {
		return window.circulo.git.push(directory, remote)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function gitCreateBranch(
	...
	if (isElectron) {
		return window.circulo.git.createBranch(directory, branchName)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function getGitRemoteUrl(directory: string, remote?: string): Promise<string | null> {
	if (isElectron) {
		return window.circulo.git.getRemoteUrl(directory, remote)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function gitApplyToLocal(
	...
	if (isElectron) {
		return window.circulo.git.applyToLocal(worktreeDir, localDir)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function gitApplyDiffText(
	...
	if (isElectron) {
		return window.circulo.git.applyDiffText(localDir, diffText)
	}
	throw new Error("Git operations are only available in Electron mode")
}
...
export async function fetchOpenInTargets(): Promise<OpenInTargetsResult> {
	if (isElectron) {
		return window.circulo.openIn.getTargets()
	}
...
export async function openInTarget(
	...
	if (isElectron) {
		return window.circulo.openIn.open(directory, targetId, persistPreferred)
	}
	throw new Error("Open-in targets are only available in Electron mode")
}
...
export async function setOpenInPreferred(targetId: string): Promise<{ success: boolean }> {
	if (isElectron) {
		return window.circulo.openIn.setPreferred(targetId)
	}
	throw new Error("Open-in targets are only available in Electron mode")
}
...
export async function fetchAutomations(): Promise<Automation[]> {
	if (isElectron) {
		return window.circulo.automation.list()
	}
...
export async function fetchAutomation(id: string): Promise<Automation | null> {
	if (isElectron) {
		return window.circulo.automation.get(id)
	}
	throw new Error("Automations are only available in Electron mode")
}
...
export async function createAutomation(input: CreateAutomationInput): Promise<Automation> {
	if (isElectron) {
		return window.circulo.automation.create(input)
	}
	throw new Error("Automations are only available in Electron mode")
}
...
export async function updateAutomation(input: UpdateAutomationInput): Promise<Automation | null> {
	if (isElectron) {
		return window.circulo.automation.update(input)
	}
	throw new Error("Automations are only available in Electron mode")
}
...
export async function deleteAutomation(id: string): Promise<boolean> {
	if (isElectron) {
		return window.circulo.automation.delete(id)
	}
	throw new Error("Automations are only available in Electron mode")
}
...
export async function runAutomationNow(id: string): Promise<boolean> {
	if (isElectron) {
		return window.circulo.automation.runNow(id)
	}
	throw new Error("Automations are only available in Electron mode")
}
...
export async function fetchAutomationRuns(automationId?: string): Promise<AutomationRun[]> {
	if (isElectron) {
		return window.circulo.automation.listRuns(automationId)
	}
	throw new Error("Automations are only available in Electron mode")
}
...
export async function archiveAutomationRun(runId: string): Promise<boolean> {
	if (isElectron) {
		return window.circulo.automation.archiveRun(runId)
	}
	throw new Error("Automations are only available in Electron mode")
}
...
export async function acceptAutomationRun(runId: string): Promise<boolean> {
	if (isElectron) {
		return window.circulo.automation.acceptRun(runId)
	}
	throw new Error("Automations are only available in Electron mode")
}
...
export async function markAutomationRunRead(runId: string): Promise<boolean> {
	if (isElectron) {
		return window.circulo.automation.markRunRead(runId)
	}
	throw new Error("Automations are only available in Electron mode")
}
