...
/**
 * Detect whether we're running inside Electron (preload injects `window.circulo`).
 */
function isElectron(): boolean {
	return typeof window !== "undefined" && "circulo" in window
}
...
export function useChromeTier() {
	...
	useEffect(() => {
		...
		window.circulo.getChromeTier().then((tier) => {
			setChromeTier(tier)
		})
	}, [setChromeTier])
	...
	useEffect(() => {
		...
		const unsubscribe = window.circulo.onChromeTier((tier: string) => {
	...
	useEffect(() => {
		if (!isElectron()) return
		document.documentElement.dataset.platform = window.circulo.platform
	}, [])
