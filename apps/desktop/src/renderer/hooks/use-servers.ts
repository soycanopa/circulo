...
export function useServerSettingsSync() {
	...
	useEffect(() => {
		...
		window.circulo.getSettings().then((settings) => {
			if (settings.servers) {
		...
		const unsub = window.circulo.onSettingsChanged((settings) => {
	...
	useEffect(() => {
		...
		window.circulo.mdns.getDiscovered().then((servers) => {
			setDiscoveredMdns(servers as DiscoveredMdnsServer[])
		})

		// Subscribe to live updates
		const unsub = window.circulo.mdns.onChanged((servers) => {
			setDiscoveredMdns(servers as DiscoveredMdnsServer[])
		})
		return unsub
	}, [setDiscoveredMdns])
...
export function useServerActions() {
	const addServer = useCallback(async (server: RemoteServerConfig, password?: string) => {
		...
		if (password) {
			await window.circulo.credential.store(server.id, password)
		}

		const settings = await window.circulo.getSettings()
		const currentServers = settings.servers?.servers ?? [DEFAULT_LOCAL_SERVER]

		await window.circulo.updateSettings({
			servers: {
	...
	const updateServer = useCallback(
		async (serverId: string, updates: Partial<RemoteServerConfig>, password?: string | null) => {
			...
			if (password !== undefined) {
				if (password === null) {
					await window.circulo.credential.delete(serverId)
				} else {
					await window.circulo.credential.store(serverId, password)
				}
			}
	...
	const testConnection = useCallback(
		async (url: string, username?: string, password?: string): Promise<string | null> => {
			...
			return window.circulo.testServerConnection(url, username, password)
		},
		[],
	)
