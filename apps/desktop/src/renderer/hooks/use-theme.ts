...
const STYLE_ID = "circulo-theme-vars"
...
export function useThemeEffect() {
	...
	useLayoutEffect(() => {
		...
		if ("circulo" in window) {
			window.circulo.setNativeTheme(colorScheme === "system" ? "system" : cls)
		}
...
export function useAvailableThemes(): ThemeDefinition[] {
	const platform =
		typeof window !== "undefined" && "circulo" in window ? window.circulo.platform : undefined
