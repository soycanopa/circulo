...
export function useNotifications(
	...
	useEffect(() => {
		...
		window.circulo.updateBadgeCount(pendingCount)
	}, [pendingCount])
	...
	useEffect(() => {
		if (!isElectron) return
		return window.circulo.onNotificationNavigate(handleNavigate)
	}, [handleNavigate])

	// --- Auto-dismiss when viewing a session ---
	useEffect(() => {
		if (!isElectron || !currentSessionId) return
		window.circulo.dismissNotification(currentSessionId)
	}, [currentSessionId])
}
