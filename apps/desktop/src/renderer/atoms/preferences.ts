...
function migrateFromZustandPersist(): void {
	const oldKey = "circulo-preferences"
	...
	try {
		...
		if (state.displayMode)
			localStorage.setItem("circulo:displayMode", JSON.stringify(state.displayMode))
		if (state.theme) localStorage.setItem("circulo:theme", JSON.stringify(state.theme))
		if (state.colorScheme)
			localStorage.setItem("circulo:colorScheme", JSON.stringify(state.colorScheme))
		if (state.drafts) localStorage.setItem("circulo:drafts", JSON.stringify(state.drafts))
		if (state.projectModels)
			localStorage.setItem("circulo:projectModels", JSON.stringify(state.projectModels))
...
function migrateDisplayMode(): void {
	const raw = localStorage.getItem("circulo:displayMode")
	if (raw === '"compact"') {
		localStorage.setItem("circulo:displayMode", '"default"')
	}
}
migrateDisplayMode()
...
export const displayModeAtom = atomWithStorage<DisplayMode>("circulo:displayMode", "default")

export const themeAtom = atomWithStorage<string>("circulo:theme", "default")

export const colorSchemeAtom = atomWithStorage<ColorScheme>("circulo:colorScheme", "dark")
 ...
 */
export const opaqueWindowsAtom = atomWithStorage<boolean>("circulo:opaqueWindows", false)
...
export const draftsAtom = atomWithStorage<Record<string, string>>("circulo:drafts", {})

export const projectModelsAtom = atomWithStorage<Record<string, PersistedModelRef>>(
	"circulo:projectModels",
	{},
)
...
export const automationsBannerDismissedAtom = atomWithStorage<boolean>(
	"circulo:automationsBannerDismissed",
	false,
)
