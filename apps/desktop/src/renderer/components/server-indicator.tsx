...
import { SidebarMenu, SidebarMenuButton, SidebarMenuItem } from "@circulo/ui/components/sidebar"
...
async function probeServerHealth(server: ServerConfig): Promise<boolean> {
	try {
		...
		if (isElectron && "circulo" in window) {
			// Use IPC fetch to bypass connection limits
			const result = await window.circulo.fetch({
