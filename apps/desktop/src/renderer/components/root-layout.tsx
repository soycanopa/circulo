 ...
 */
import { TooltipProvider } from "@circulo/ui/components/tooltip";
...
export function RootLayout() {
	...
	const showOnboarding =
		isElectronEnv && !onboardingState.completed && !isMockMode;
	...
	const handleKeyDown = useCallback(
		(e: KeyboardEvent) => {
			...
			if (
				...
				if (e.key === "j") {
					nextIndex =
						currentIndex < visibleAgents.length - 1 ? currentIndex + 1 : 0;
				} else {
					nextIndex =
						currentIndex > 0 ? currentIndex - 1 : visibleAgents.length - 1;
				}
		...
		[sessionId, visibleAgents, navigate, setCommandPaletteOpen, setChatInitDir],
	);
	...
	const contentReady =
		phase === "ready" || phase === "loading-sessions" || phase === "error";
