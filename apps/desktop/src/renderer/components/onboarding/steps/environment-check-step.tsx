...
export function EnvironmentCheckStep({ onComplete, onSkip }: EnvironmentCheckStepProps) {
	...
	const runChecks = useCallback(async () => {
		...
		try {
			...
			const result = await window.circulo.onboarding.checkOpenCode()
	...
	const handleInstall = useCallback(async () => {
		...
		const cleanup = window.circulo.onboarding.onInstallOutput((text) => {
			setInstallOutput((prev) => [...prev, text])
		})

		try {
			const result = await window.circulo.onboarding.installOpenCode()
			cleanup()
			...
			<div className="w-full max-w-lg space-y-6">
				<div className="text-center">
					...
					<p className="mt-1 text-sm text-muted-foreground">
						Verifying your setup is ready for Circulo.
					</p>
				</div>
					...
					>
						<p className="text-sm text-muted-foreground">
							{needsUpdate
								...
								: "Circulo needs the OpenCode CLI to function. Install it to continue."}
						</p>
