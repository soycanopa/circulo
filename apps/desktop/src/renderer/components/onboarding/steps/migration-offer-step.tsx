...
import { Checkbox } from "@circulo/ui/components/checkbox"
...
export function MigrationOfferStep({ provider, onPreview, onSkip }: MigrationOfferStepProps) {
	...
	const handlePreview = useCallback(async () => {
		...
		try {
			const preview = await window.circulo.onboarding.previewMigration(
				provider,
			...
			<div className="w-full max-w-lg space-y-6">
				<div className="text-center">
					...
					<p className="mt-1 text-sm text-muted-foreground">
						We detected an existing {label} setup. Circulo can migrate your configuration to OpenCode
						format.
					</p>
				</div>
