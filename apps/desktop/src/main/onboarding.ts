/**
 * Onboarding handlers for the Circulo desktop app.
 *
 ...
 * - Multi-provider config detection and migration via @circulo/configconv
 ...
 * Does NOT import @circulo/configconv, just checks for file/directory existence.
...
// ============================================================
// Migration (lazy-loads @circulo/configconv)
// ============================================================

/**
 * Runs a full scan for the specified provider and returns detailed detection results.
 * Lazy-loads @circulo/configconv to keep the main process fast when not needed.
 */
export async function scanProvider(provider: MigrationProvider): Promise<{
	detection: ProviderDetection
	scanResult: unknown
}> {
	const { scanFormat } = await import("@circulo/configconv")
...
export async function previewMigration(
	...
	const { universalConvert } = await import("@circulo/configconv")

	// Convert from source provider to OpenCode (the target for Circulo)
...
export async function executeMigration(
	...
	const { universalConvert, universalWrite } = await import("@circulo/configconv")
...
async function executeHistoryMigration(
	...
	if (provider === "cursor" && result?.data?.history) {
		const { convertCursorHistory } = await import("@circulo/configconv/converter/cursor-history")
		const { writeHistorySessionsDetailed } = await import("@circulo/configconv/writer/history")
		...
		const { convertHistory } = await import("@circulo/configconv/converter/history")
		const { writeHistorySessionsDetailed } = await import("@circulo/configconv/writer/history")
...
export async function restoreMigrationBackup(): Promise<{
	...
	const { restore } = await import("@circulo/configconv")
