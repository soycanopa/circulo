 ...
 *
 * macOS packaged:  Circulo.app/Contents/Resources/bin/circulo
 * Linux packaged:  <install-dir>/resources/bin/circulo
 * Windows:         <install-dir>/resources/bin/circulo.cmd
 * Dev mode:        <repo>/apps/desktop/resources/bin/circulo
 */
function getCliSourcePath(): string {
	const scriptName = process.platform === "win32" ? "circulo.cmd" : "circulo"
 ...
 * Windows:     %LOCALAPPDATA%\Circulo\bin  (added to PATH by the user)
 */
function getCliInstallDir(): string {
	if (process.platform === "win32") {
		const localAppData =
			process.env.LOCALAPPDATA ?? path.join(app.getPath("home"), "AppData", "Local")
		return path.join(localAppData, "Circulo", "bin")
	}
	return "/usr/local/bin"
}

/**
 * Returns the full path where the `circulo` symlink/script will be placed.
 */
function getCliInstallPath(): string {
	const name = process.platform === "win32" ? "circulo.cmd" : "circulo"
...
/**
 * Installs the `circulo` CLI command by symlinking it into /usr/local/bin
...
/**
 * Uninstalls the `circulo` CLI command by removing the symlink.
 */
