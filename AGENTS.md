...
- **`preload/`** -- Electron preload bridge: exposes `window.circulo` API via `contextBridge`
...
- Renderer: `@circulo/ui` -> `@tanstack/*` -> `lucide-react` -> `react` -> local atoms/hooks/services
...
The main process runs in Node.js, the renderer runs in a Chromium sandbox. They communicate via IPC only. Never import Node.js modules (`fs`, `child_process`, `path`) in the renderer -- use the `window.circulo` bridge or `services/backend.ts` instead.

### Backend Service Layer -- `services/backend.ts`

All hooks must import from `services/backend.ts`, NOT from `services/circulo-server.ts` directly. The backend module detects Electron (`"circulo" in window`) and routes to IPC or HTTP automatically.
...
The `window.circulo` bridge is not available until the preload script finishes. Early-running renderer code (e.g., module-level calls, top-of-file side effects) must guard with optional chaining: `window.circulo?.someMethod()`.
...
### Circulo storage -- XDG Base Directory

Circulo follows the XDG Base Directory Specification (same convention as OpenCode). Config at `~/.config/circulo/`, data at `~/.local/share/circulo/`. Automation configs live at `~/.config/circulo/automations/<id>/`, SQLite database at `~/.local/share/circulo/circulo.db`. See `main/automation/paths.ts` for the implementation. Do NOT use `~/.circulo/` (legacy) or Electron's `userData` path for automation storage.
