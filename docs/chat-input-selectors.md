...
On session switch, the selectors are immediately seeded from `projectModelsAtom` -- a Jotai atom backed by `localStorage` under the key `circulo:projectModels`.

```
chat-view.tsx:525-542

Source: atoms/preferences.ts:84-87
  projectModelsAtom = atomWithStorage<Record<string, PersistedModelRef>>("circulo:projectModels", {})
...
Electron mode:  window.circulo.updateModelRecent(model)
...
This writes to `localStorage` under `circulo:projectModels`, keyed by the project directory. The persisted shape is:
...
| `renderer/services/circulo-server.ts` | HTTP implementations for model state (browser mode) |
...
| `preload/index.ts` | Exposes `getModelState` and `updateModelRecent` on `window.circulo` bridge |
